        ...
        <div className="mb-16 grid gap-8 lg:grid-cols-2">
          {projects.map((project, index) => (
            <Reveal key={project.name} delayMs={index * 80}>
              <article className="group relative h-full overflow-hidden rounded-[1.5rem] border border-line bg-white/55 p-7 backdrop-blur-sm transition duration-300 hover:-translate-y-1 hover:border-sea/35 hover:bg-white/80 sm:p-8">
                <div className="absolute -right-10 -top-10 h-36 w-36 rounded-full bg-sea/15 blur-2xl transition duration-500 group-hover:bg-signal/25" />
                <p className="mono-label relative break-words">{project.org}</p>
                ...
                <p className="relative mt-4 text-base font-light leading-relaxed text-ink-soft">
                  {project.description}
                </p>
          ...
          {experience.map((job) => {
            ...
            return (
              <Reveal key={job.company}>
                <article className="grid gap-6 border-b border-line py-10 md:grid-cols-[minmax(0,0.9fr)_minmax(0,1.4fr)] md:gap-10">
                  <div>
                    ...
                    {job.current ? (
                      <span className="mt-4 inline-flex items-center gap-2 rounded-full bg-signal/20 px-3 py-1 text-xs font-semibold text-signal-ink">
                  ...
                  <ul className="space-y-3 text-[0.98rem] font-light leading-relaxed text-ink-soft">
